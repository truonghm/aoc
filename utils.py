...
def read_input(day, year) -> str:
	try:
		with open(f"input/{year}/input_{year}{str(day).zfill(2)}.txt", "r") as f:
			return f.read()
	except:
		raise FileNotFoundError(f"The provided day ({day}) and/or year ({year}) does not match any input file.")

def download_input(day, year) -> str:
	...
	input_path = f"input/{year}/input_{year}{str(day).zfill(2)}.txt"
